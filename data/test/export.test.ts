...
import { assembleMarketRows, exportAll } from "../src/exporters.ts";
...
describe("exportAll", () => {
  it("writes csv + parquet with proper escaping", async () => {
    const { files } = await exportAll(handle.db, tmp);
    for (const f of files) expect(existsSync(f)).toBe(true);
    const csv = readFileSync(path.join(tmp, "markets.csv"), "utf8");
    expect(csv.split("\n")[0]).toContain("market_id,external_id,question");
